...
void fit_2D_bc2()
{
  ...
  TChain chain("h1");
  chain.Add("/home/souvik/leptoquark_analysis/elec_mode/d0pmep_mode/evtgen_exp_*.root");
  Float_t d0mass,mass,mom,delta,dsmom,dstrf,dz,e1_l,prk1_l,prp1_l,elec_mom,pr_mom,pissvd,elsvd,prsvd,pis_k,bc3,bc2,pis_p ;
  ...
  for(int i=0;i< chain.GetEntries();i++) {
    ...
    deltam->setVal(delta);
    md0->setVal(d0mass);
    if(d0mass>1.8 && d0mass<1.9 && delta>0.14 && delta<0.152 && e1_l>0.9 && prk1_l>0.6 && prp1_l>0.6 && q<0.02 && mom>2.5 && pis_p>0.6 && pissvd>2 && prsvd>2 && elsvd>2 && elec_mom>0.7 && pr_mom>1.7 && bc2==100)
      data->add(RooArgSet(*deltam,*md0));
  }
  
  /*****************************Fit***********************/
  RooRealVar *mean1_dm = new RooRealVar("mean1_dm", "MEAN of 1st gaussian",0.145,0.14,0.152);
  RooRealVar *sigma1_dm = new RooRealVar("sigma1_dm", "Sigma of 1st gaussian",0.0002410,0,0.0089);
  RooGaussian *gauss_dm= new RooGaussian("gauss_dm", "1st gaussian PDF", *deltam, *mean1_dm, *sigma1_dm);        
  RooRealVar *delm1_dm=new RooRealVar("delm1_dm", "Difference of Mean",0.0000596,-0.01,0.01);
  ...
  RooRealVar *sigma2_dm = new RooRealVar("sigma2_dm", "Sigma of 2nd gaussian",0.002668,0,0.0089);
  RooRealVar *sigma3_dm = new RooRealVar("sigma3_dm", "Sigma of 2nd gaussian",0.000194,0,0.0089);
  RooRealVar *sigma4_dm = new RooRealVar("sigma4_dm", "Sigma of 2nd gaussian",0.000703,0,0.0089);
  ...
  RooRealVar *a2a_dm1=new RooRealVar("a2a_dm1", " Area2/Area1",0.344,0.1,1);
  RooRealVar *a2a_dm=new RooRealVar("a2a_dm", " Area2/Area1",0.858,0.1,1);
  RooRealVar *a2a_dm2=new RooRealVar("a2a_dm2", " Area2/Area1",0.858,0.01,1);
  ...
  // M_D0 fit
  RooRealVar *mean1_md = new RooRealVar("mean1_md", "MEAN of 1st gaussian",1.864695,1.8,1.9);
  ...
  RooRealVar *sigma_md = new RooRealVar("sigma_md", "Sigma of 1st gaussian",0.005,0,0.05);
  ...
  RooRealVar *srs1_md=new RooRealVar("srs1_md","Ratio of Sigma",0.85,0.1,10);
  RooRealVar *a2a_md=new RooRealVar("a2a_md", " Area2/Area1",0.01,1);
  RooRealVar *delm1_md=new RooRealVar("delm1_md", "Difference of Mean",0.00180,-0.01,0.01);
  ...
  RooRealVar * slope1= new RooRealVar("slope1", "Slope of Polynomial",-0.2407,-1,1);
  RooRealVar * slope2= new RooRealVar("slope2", " Slope of Cheb2",-0.2518,-1,1);
  ...
  RooRealVar *BKG = new RooRealVar("BKG", "",200,300000);
  ...
  TLatex s;
     ...
     s.SetTextFont(42);
     s.SetTextColor(2);
     s.DrawLatex(0.3,0.5,"#chi^{2}/ndf = 0.95");
  ...
  yframe->Draw() ;
   ...
   TLatex l;
     ...
     l.SetTextFont(42);
     l.SetTextColor(2);
     l.DrawLatex(0.3,0.5,"#chi^{2}/ndf = 1.38");
