...
void fit_2D_bc3()
{
  ...
  Float_t d0mass,mass,mom,delta,dsmom,dstrf,dz,e1_l,prk1_l,prp1_l,elec_mom,pr_mom,pissvd,elsvd,prsvd,pis_k,bc3,bc0,pis_p ;
  ...
  for(int i=0;i< chain.GetEntries();i++) {
    ...
    deltam->setVal(delta);
    md0->setVal(d0mass);
    if(d0mass>1.8 && d0mass<1.9 && delta>0.14 && delta<0.152 && e1_l>0.9 && prk1_l>0.6 && prp1_l>0.6 && q<0.02 && mom>2.5 && pis_p>0.6 && pissvd>2 && prsvd>2 && elsvd>2 && elec_mom>0.7 && pr_mom>1.7 && bc3==100)
  ...
  // M_D0 fit
  RooRealVar *mean1_md = new RooRealVar("mean1_md", "MEAN of 1st gaussian",1.864,1.8,1.9);
  RooRealVar *sigma1_md = new RooRealVar("sigma1_md", "Sigma of 1st gaussian",0.003218,0,0.0089);
  ...
  RooRealVar *sigma_md = new RooRealVar("sigma_md", "Sigma of 1st gaussian",0.00699,0,0.05);
  ...
  RooRealVar *delm1_md=new RooRealVar("delm1_md", "Difference of Mean",0.00336,-0.01,0.01);
  ...
  RooRealVar * slope1= new RooRealVar("slope1", "Slope of Polynomial",-0.3273,-1,1);
  RooRealVar * slope2= new RooRealVar("slope2", " Slope of Cheb2",-0.1534,-1,1);
  ...
  TLatex s;
     ...
     s.SetTextFont(42);
     s.SetTextColor(2);
     s.DrawLatex(0.3,0.5,"#chi^{2}/ndf = 1.23");
