...
void d0mass_tm()
{
  ...
  Float_t mult_ds, dstrf, d0f,d0mass,mass,mom,deltam,elf,prf,elec_mom,pr_mom,elec_mas,pr_mass,mult_d0,e1_l,prk1_l,prp1_l,pis_k,pis_p,pissvd,prsvd,elsvd,bc0,bc1,modedz,modeadz ;
  ...
  t1->SetBranchAddress("pis_p",&pis_p);
  ...
  t1->SetBranchAddress("modedz",&modedz);
  t1->SetBranchAddress("modeadz",&modeadz);
  
  /*  
  TH1F *hist3 = new TH1F("hist3","M_{D^{0}}",80, 1.8,1.9);
  for (Int_t i=0; i < t1->GetEntries(); ++i)
    {
      t1->GetEntry(i);
      double q = mass-d0mass-0.139 ;
      {
	if(deltam>0.144 && deltam<0.147 && mom>2.5 && q<0.02 && e1_l>0.9 && prk1_l>0.7 && prp1_l>0.7 && pis_p>0.6 && pissvd>2 && prsvd>2 && elsvd>2 && bc0==100)
	hist3->Fill(d0mass);
      }
    }

  hist3->GetYaxis()->SetRangeUser(0,30000);
  hist3->GetXaxis()->SetTitle("M_{D^{0}}[GeV/c^{2}]");
  ...
  hist3->Draw();
  */
  TH1F *hist4 = new TH1F("hist4","M_{D^{0}}",80, 1.8,1.9);
  for (Int_t i=0; i < t1->GetEntries(); ++i)
    {
      t1->GetEntry(i);
      double q = mass-d0mass-0.139 ;
      {
	if(dstrf==1 && deltam>0.144 && deltam<0.147 && mom>2.5 && q<0.02 && e1_l>0.9 && prk1_l>0.7 && prp1_l>0.7 && pis_p>0.6 && pissvd>2 && prsvd>2 && elsvd>2 && bc0==100)
        hist4->Fill(d0mass);
      }
    }

  hist4->GetYaxis()->SetRangeUser(0,30000);
  hist4->GetXaxis()->SetTitle("M_{D^{0}}[GeV/c^{2}]");
  ...
  TH1F *hist5 = new TH1F("hist5","M_{D^{0}}",80, 1.8,1.9);
  for (Int_t i=0; i < t1->GetEntries(); ++i)
    {
      t1->GetEntry(i);
      double q = mass-d0mass-0.139 ;
  ...
  hist5->GetYaxis()->SetRangeUser(0,30000);
  hist5->GetXaxis()->SetTitle("M_{D^{0}}[GeV/c^{2}]");
