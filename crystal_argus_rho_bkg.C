...
void crystal_argus_rho_bkg() {
  ...
  RooRealVar *sigma1 = new RooRealVar("sigma1", "Sigma of 1st gaussian",0,0.0089);
  ...
  RooRealVar *mean = new RooRealVar("mean", "MEAN of 1st gaussian",1.864,1.8,1.9);
  RooRealVar *sigma = new RooRealVar("sigma", "Sigma of 1st gaussian",0,0.05);
  RooRealVar *alpha = new RooRealVar("alpha", "alpha",1,0.1,4);
  RooRealVar *n = new RooRealVar("n", "Polynom degree",14); //14,1,20)
  RooFormulaVar *mean3= new RooFormulaVar("mean3","Mean of 2nd gaussian", "@0+@1",RooArgList(*mean1,*delm1));
  RooCBShape *gauss1_1 = new RooCBShape("gauss1_1","Crystal Ball shape", *deltam, *mean3, *sigma, *alpha, *n);

  RooAddPdf *dG_1= new RooAddPdf("dG_1", " 1st GAuss + 3rd Gauss", RooArgList(*gauss,*gauss1_1), RooArgList(*a2a));
  ...
  RooRealVar *BKG = new RooRealVar("BKG", "",35,300000);
  ...
  depdf->plotOn(frame1,Components(RooArgSet(*chebpol)),LineColor(kGreen),LineStyle(kDashed));                           
  //depdf->plotOn(frame1,Components(RooArgSet(*argus)),LineColor(kCyan),LineStyle(kDashed));
