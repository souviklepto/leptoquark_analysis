...
void mom()
{
  ...
  for (Int_t i=0; i < t1->GetEntries(); ++i)
    {
      ...
      {
	if(deltam>0.14 && deltam<0.16 && q<0.02 && e1_l>0.9 && prk1_l>0.9 && prp1_l>0.9 && pis_k<0.4 && pissvd>2 && prsvd>2 && elsvd>2 && bc0==100)
	hist3->Fill(mom);
      }
    }

  hist3->GetYaxis()->SetRangeUser(0,15000);
  hist3->GetXaxis()->SetTitle("P_{D^{*}}[GeV/c]");
  ...
  for (Int_t i=0; i < t1->GetEntries(); ++i)
    {
      ...
      double q = mass-d0mass-0.139 ;
      {
	if(dstrf==1 && deltam>0.14 && deltam<0.16 && q<0.02 && e1_l>0.9 && prk1_l>0.9 && prp1_l>0.9 && pis_k<0.4 && pissvd>2 && prsvd>2 && elsvd>2 && bc0==100)
        hist4->Fill(mom);
      }
    }
  ...
  TH1F *hist5 = new TH1F("hist5","M_{D^{0}}",80, 0,5);
  for (Int_t i=0; i < t1->GetEntries(); ++i)
    {
      t1->GetEntry(i);
      double q = mass-d0mass-0.139 ;
  ...
  hist5->GetYaxis()->SetRangeUser(0,15000);
  hist5->GetXaxis()->SetTitle("P_{D^{*}}[GeV/c]");
