...
void fit_2D()
{
  ...
  TChain chain("h1");
  chain.Add("/home/souvik/leptoquark_analysis/elec_mode/elec_signalMC/evtgen_exp_*.root");
  ...
  for(int i=0;i< chain.GetEntries();i++) {
    ...
    deltam->setVal(delta);
    md0->setVal(d0mass);
    if(d0mass>1.8 && d0mass<1.9 && delta>0.14 && delta<0.152 && e1_l>0.9 && prk1_l>0.6 && prp1_l>0.6 && q<0.02 && mom>2.5 && pis_p>0.6 && pissvd>2 && prsvd>2 && elsvd>2 && elec_mom>0.7 && pr_mom>1.7 && bc0==100)
  ...
  RooRealVar *delm1_md=new RooRealVar("delm1_md", "Difference of Mean",0.00428,-0.01,0.01);
  ...
  RooRealVar * slope1= new RooRealVar("slope1", "Slope of Polynomial",-0.3073,-1,1);
  RooRealVar * slope2= new RooRealVar("slope2", " Slope of Cheb2",-0.4561,-1,1);
  ...
  TLatex s;
     ...
     s.SetTextFont(42);
     s.SetTextColor(2);
     s.DrawLatex(0.3,0.5,"#chi^{2}/ndf = 1.18");
  ...
  yframe->Draw() ;
   ...
   TLatex l;
     ...
     l.SetTextFont(42);
     l.SetTextColor(2);
     l.DrawLatex(0.3,0.5,"#chi^{2}/ndf = 1.54");
